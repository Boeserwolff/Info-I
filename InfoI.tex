...
{morekeywords={two,three,four,five,six,seven,eight,
...
\end{lstlisting}	
Konstruktion von Funktionen \la f\ra, die zusammengesetzte Daten der Signatur \la t\ra \uline{konsumiert}.\\
\begin{itemize}
\item Welchen Record-Komponenten $\langle comp_i \rangle$ sind relevant für \la f\ra?
\item $\Rightarrow$ Schablone:
\begin{lstlisting}[style=customc]
(:§\la§f§\ra§(... §\la§t§\ra§ ... §$\rightarrow$§ ...))
(define §\la§f§\ra§
  (lambda (... r ...)
   ... (§\la§t§\ra§-§\la $comp_i$\ra§ r)...))
(: not (boolean -> boolean))
\end{lstlisting}
\end{itemize}
...
\node [rectangle,draw] (z){\ \ \ |\ \ \ \ }
	...
	child{node[rectangle,draw]{\ \ \ |\ \ \ \ }
		...
		child{node{empty}edge from parent[green]}edge from parent[green]}
	;
\end{tikzpicture}
...
$t \equiv \Downarrow$\\ 
\begin{tikzpicture}[thick,level distance=.8cm,level 1/.style={sibling distance=1cm}]
\draw (0,-1) -- (1,-1) -- (0.5,0) -- (0,-1);
\draw (0.15,-0.9) circle [radius=0.1cm] node [below] {lm};
\draw (5,0) node {(leftmost};
\draw (6.5,0) node [circle,draw]{x} child{node[rectangle,draw] {}}child{node{?}};
\draw (8,0) node {) = (list x)};
\draw (5,-1.5) node {(leftmost};
\draw (6.5,-1.5) node [circle,draw]{x} child{node[rectangle,draw] {$l$}}child{node{?}};
\draw (8,-1.5) node {) = (leftmost};
\draw (9.55,-1.5) node [rectangle,draw] {$l$};
\draw (9.9,-1.5) node {)};
\end{tikzpicture}
\begin{lstlisting}[style=customc]
...
child{node[rectangle,draw]{}}
child{node[rectangle,draw]{}}};
\draw (4.5,-1) node {Daraus wird die Liste 2 1 3};
\end{tikzpicture}\\
\subsubsection{Beispiel}
Baumdarstellung des arithmetischen Ausdrucks (Term)\\
$2+3\cdot 4 \leftarrow$ $\cdot$ bindet stärker als +\\
 \begin{tikzpicture}[thick,level distance=1.2cm,level 2/.style={sibling distance=1.5cm},level 1/.style={sibling distance=2.5cm},level 3/.style={sibling distance=1cm}]
 \node [circle,draw] {+}
 child{node[circle,draw] {2}
 child{node[rectangle,draw]{}}
 child{node[rectangle,draw]{}}}
 ...
 child{node[rectangle,draw]{}}}
  ...
  child{node[rectangle,draw]{}}}};
 \end{tikzpicture}\\
Ein Breitendurchlauf (breath-first traversal) eines Baumens t sammelt die Markierungen der Knoten \uline{ebenenweise} von der Wurzel ausgehend auf.\\
t$\equiv$
 \begin{tikzpicture}[thick,level distance=1.2cm,level 2/.style={sibling distance=1.5cm},level 1/.style={sibling distance=3cm},level 3/.style={sibling distance=0.75cm}]
 \node [circle,draw] {s}
 ...
 child{node[rectangle,draw]{}}}
  ...
  child{node[rectangle,draw]{}}
  child{node[rectangle,draw]{}}}};
\draw [orange,thick](-2.5,-.5) -- (3,-.5) node [right,above] {Ebenen};
\draw [orange,thick](-2.5,-1.75) -- (3,-1.75);
\draw [orange,thick](-2.5,-3) -- (3,-3);
\draw [orange,thick](-2.5,-4) -- (3,-4);
\end{tikzpicture}\\
\begin{lstlisting}[style=customc]
...
\begin{itemize}
\item[(1)] Sammle die Liste der Markierungen der Wurzeln der (nicht-leeren) Bäume n ts (= (roots ts))
\item[(2)] Bestimme die Liste ts' der Teilbäume der (nicht-leeren) Bäume n ts (= (subtrees ts))
\item[(3)] Führe (1) rekursiv auf ts' aus.
\item[(4)] komkatiniere die Listen aus (1) und (3).
\end{itemize}    
\uline{Zu Beginn}\\
 \begin{tikzpicture}[thick,level distance=1.2cm,level 2/.style={sibling distance=1.5cm},level 1/.style={sibling distance=3cm},level 3/.style={sibling distance=0.75cm}]
 \node [circle,draw] {s}
 ...
 child{node[rectangle,draw]{}}}
  ...
  child{node[rectangle,draw]{}}
  child{node[rectangle,draw]{}}}};
\draw [orange,thick] (3.5,0) node {roots}; %klammer um knoten s 
\draw [orange,thick] (-0.5,0) node {(};
\draw [orange,thick] (0.5,0) node {)};
\draw [orange,thick] (4,-1) node {subtrees}; %klammer um knoten von subtrees dazischen komma
\draw [orange,thick] (-2.2,-1.2) node {(};
\draw [orange,thick] (0,-1.2) node {,};
\draw [orange,thick] (2.2,-1.2) node {)};
\end{tikzpicture}\\
\uline{1. Rekursionaufruf}\\
\begin{tikzpicture}[thick,level distance=1.2cm,level 1/.style={sibling distance=1.5cm},level 2/.style={sibling distance=0.75cm}]
\node[circle,draw] {c}
 child{node[rectangle,draw]{}}
  child{node[circle,draw] {e}
  child{node[rectangle,draw]{}}
  child{node[rectangle,draw]{}}};
\draw (2.7,0) node[circle,draw] {h}
 child{node[circle,draw] {m}
 child{node[rectangle,draw]{}}
 child{node[rectangle,draw]{}}}
  child{node[circle,draw] {e}
  child{node[rectangle,draw]{}}
  child{node[rectangle,draw]{}}};
  \draw [orange,thick] (4,0) node {roots}; %klammer um knoten s 
  \draw [orange,thick] (-0.5,0) node {(};
  \draw [orange,thick] (1.5,0) node {,};
  \draw [orange,thick] (3.2,0) node {)};
  
  \draw [orange,thick] (5,-1) node {subtrees}; %klammer um knoten von subtrees dazischen komma
  \draw [orange,thick] (-1,-1.2) node {(};


  \draw [orange,thick] (0,-1.2) node {,};
  \draw [orange,thick] (1.4,-1.2) node {,};
  \draw [orange,thick] (2.8,-1.2) node {,};
  \draw [orange,thick] (4,-1.2) node {)};
\end{tikzpicture}\\
%tikzpicture mit c und h dann selbe lable wie subtrees vorher
\uline{2. Rekursionaufruf}\\
\begin{tikzpicture}[thick,level distance=1.2cm,level 1/.style={sibling distance=0.75cm}]
\node[circle,draw] {e}
  child{node[rectangle,draw]{}}
  child{node[rectangle,draw]{}};
\draw (1.25,0) node[circle,draw] {m}
  child{node[rectangle,draw]{}}
  child{node[rectangle,draw]{}};
\draw (2.5,0) node[circle,draw] {e}
  child{node[rectangle,draw]{}}
  child{node[rectangle,draw]{}};
  
    \draw [orange,thick] (4,0) node {roots}; %klammer um knoten s 
    \draw [orange,thick] (-0.5,0) node {(};
    \draw [orange,thick] (0.6,0) node {,};
    \draw [orange,thick] (2,0) node {,};
    \draw [orange,thick] (3.2,0) node {)};
    
    \draw [orange,thick] (5,-1) node {subtrees}; %klammer um knoten von subtrees dazischen komma
    \draw [orange,thick] (-0.8,-1.2) node {(};
    \draw [orange,thick] (0,-1.2) node {,};
    \draw [orange,thick] (0.65,-1.2) node {,};
    \draw [orange,thick] (1.3,-1.2) node {,};
    \draw [orange,thick] (1.9,-1.2) node {,};
    \draw [orange,thick] (2.5,-1.2) node {,};
    \draw [orange,thick] (3.2,-1.2) node {)};
      \draw (-0.6,-1) -- (-0.2,-1.4);
      \draw (0.2,-1) -- (0.6,-1.4);
      \draw (0.7,-1) -- (1.1,-1.4);
      \draw (1.4,-1) -- (1.8,-1.4);
      ...
      \draw (2.7,-1) -- (3.1,-1.4);
\end{tikzpicture}\\
%tikzpicture mit e und m und e dann selbe lable wie subtrees vorher (gibt nur roots)
\uline{3. Rekursionsaufruf}\\
\begin{tikzpicture}
\draw (0,0);
\draw (2,0) node {( )};
\end{tikzpicture}
...
\end{lstlisting}
\section{Neue Sprachebene DMdA-fortgeschritten}
\begin{itemize}
\item neues Ausgabeformat in der REPL (list $x_1\ x_2\ \ x_n$) $\rightsquigarrow$ ($x_1 \ x_2 \ \ x_n$)\\
      empty $\rightsquigarrow$ ()\\
\item Neuer  Gleichheitstest für Werte aller (auch benutzerdef.) Signaturen (:equal? ( \%a \%b -> boolean))
\end{itemize}
\subsection{Quote}
...
\uline{Beispiel:}\\
e $\equiv$ '(* (! (+ 1 2)) x) %Pfeil zu ! * x + symbols Pfeil zu 1 2 numbers
\begin{lstlisting}[style=customc]
(define term
...
Auswertung eines arithmetischen Ausdrucks e (im Enviroment d):\\
 ((eval d) e)
\begin{itemize}
\item[($E_1$)] ((eval d) c) = c % c Konstante
\item[($E_2$)] ((eval \{$x_1\rightarrow v_1, \ \ \ ,x_n \rightarrow v_n$\})$x_i$) = $v_i$ %X_i Variable
\item[($E_3$)] ((eval d) ($e_1 \ e_2 \ ... \ e_n$)) = (... (((eval d) $e_1$) ((eval d) $e_2$))...((eval d) $e_n$))
\end{itemize}
\begin{tikzpicture}[thick,level distance=1.2cm,level 1/.style={sibling distance=2cm},level 2/.style={sibling distance=2.5cm}]
\node {@}
  child{node {@}
  child{node{((eval d) $e_1$)}}
  child{node{((eval d) $e_2$)}}}
  child{node{((eval d) $e_n$)}};
\end{tikzpicture}
\begin{lstlisting}[style=customc]
...
\item $\forall e_1,e_2\in E:$ ($e_1 e_2)\in E$ [Applikation] % e1 funktion e2 argument
\item $\forall v \in V, e_1\in E:$ ($\lambda v.e_1)\in E$ [Abstraktion] %v Parameter e1 Rumpf
\end{itemize}
\uline{Beispiele}
$y\in E$\\
...
\item[($\rightarrowtail_5$)] %Große klammer
